...
function filterByRange<T extends { date: string }>(points: T[], range: RangeLabel, anchor: Date): T[] {
  ...
  const cutoff = new Date(anchor.getTime() - days * 24 * 3600 * 1000);
...
/** Ported from portfolio-watcher's PremiumChart, trimmed to line/bar only
 * (no candlestick/compare) since options mark-price series don't carry OHLC. */
  ...
  return (
    <div>
          ...
          {RANGES.map((r) => (
            <button
              ...
              className={`rounded-md px-2 py-1 text-xs font-semibold transition-all duration-150 ease-out active:scale-[0.95] ${
                range === r.label ? "bg-teal-600 text-white shadow-sm" : "text-ink-500 hover:-translate-y-0.5 hover:text-ink-900"
              }`}
            >
              {r.label}
            </button>
      ...
      <div className="mb-3 flex gap-1">
        {(Object.keys(TYPE_ICONS) as ChartType[]).map((t) => (
          <button
            key={t}
            onClick={() => setChartType(t)}
            className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-semibold capitalize transition-all duration-150 ease-out active:scale-[0.95] ${
              chartType === t
          ...
          (() => {
            ...
            return (
              <div
                ...
                style={{ left: `${(x / WIDTH) * 100}%`, top: `${(y / HEIGHT) * 100}%`, transform: "translate(-50%, -145%)" }}
              >
